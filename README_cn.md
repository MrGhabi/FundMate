...
- 📘 [Developer Guide](AGENTS.md#fundmate-developer-guide) — 详细架构、目录规范、CLI/TC/Web 说明、环境变量、排障（英文）。
- 🤖 [Agent Onboarding](AGENTS.md#agent-onboarding-notes-for-fundmate) — 针对智能体/自动化流程的管线与测试说明。

## 核心特性

- **多格式解析**：PDF（LLM OCR）与 Excel 对账单统一成 `Position` 对象与现金科目。
- **交易确认书模式**：在基准组合上增量应用 TC，内置港股数字期权映射、基线缓存。
- **实时定价**：优先 Futu OpenD；`akshare` 通路存在但已废弃（deprecated）。货币市场基金自动归类为现金。
...
Web 界面读取 `./out/result/<date>` 的结果文件。生产部署、模板/静态资源打包细节见 [AGENTS.md](AGENTS.md#6-web-ui)（英文）。

---

目录结构、所有 CLI 选项、交易确认书基线、环境变量、数据归档脚本等完整内容，已合并至 [Developer Guide](AGENTS.md#fundmate-developer-guide)。此 README 仅保留入口与常用命令，方便快速上手。
